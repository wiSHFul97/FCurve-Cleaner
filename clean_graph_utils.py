...
def shorten_the_range1(points, period, from_start=False, is_min=True, tolerance=1): 
...
def shorten_the_range(points, period, from_start=False, is_min=True, anomaly_count=2): 
	anomalies = 0
...
def break_range(points, range_start, point1, range_end, point2, anomaly_count):
	...
	first_range = shorten_the_range1(points, (range_start, point1[0]), from_start=False, is_min=point1_is_min, tolerance=anomaly_count)
	...
	second_range_start = shorten_the_range1(points, (point1[0], point2[0]), from_start=True, is_min=point1_is_min, tolerance=anomaly_count)
	second_range_end = shorten_the_range1(points, (point1[0], point2[0]), from_start=False, is_min=(not point1_is_min), tolerance=anomaly_count)
	...
	last_range = shorten_the_range1(points, (point2[0], range_end), from_start=True, is_min=(not point1_is_min), tolerance=anomaly_count)
...
def get_lr_exterms_kf_inds(points, start, end, anomaly_count=2):
	selected_kf_inds = []
	prev_start = start
	prev_end = end
	while True:
		new_start, new_end = get_new_start_end(points, prev_start, prev_end, anomaly_count=anomaly_count)
...
def get_new_start_end(points, start, end, anomaly_count=2):
	last_frame_is_min = points[end-1][1].y > points[end][1].y
	new_end = shorten_the_range1(points, (start, end), from_start=False, is_min=last_frame_is_min, tolerance=anomaly_count)[1]

	first_frame_is_min = points[start+1][1].y > points[start][1].y # TODO noise problem
	new_start = shorten_the_range1(points, (start, end), from_start=True, is_min=first_frame_is_min, tolerance=anomaly_count)[0]
...
def find_inbetween_points(selected_kf_inds, points, inbetween_precision):
	...
	for ind in range(len(selected_kf_inds)-1):
		...
		# print(straighted_points) TODO is convert_to_straight_line correct?
		# TODO why len - 2 ?
		# inds = get_min_max_kf_inds(straighted_points, new_start, new_end, anomaly_count=2) # TODO -2?
		inds = get_lr_exterms_kf_inds(straighted_points, 0, len(straighted_points)-1, anomaly_count=inbetween_precision)
